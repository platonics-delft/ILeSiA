...
This folder is a ros package that is used to save and load the demonstrations. We used this folder to have a ligher repository and save all the demonstration in this other one. 
...
> How is solved the discrepacy? Box is on different place than on recording, so task state configuration won't work, they can be transofrmed based on the image from the recording and real time.
...
> It uses template only when searching for box and finding initial transformation of the box. During the execution it is not used, becase demonstrated images are used comparing the actual ones 
